...
END $$
DELIMITER ;
SELECT salary from employees e join departments d on e.department_id = d.department_id where d.name = 'Engineering' ORDER BY salary desc;
...
BEGIN 
	START TRANSACTION;
		IF ((SELECT count(*) FROM employees WHERE employee_id = id) > 0) THEN 
...
SELECT * from deleted_employees ORDER BY employee_id; 
DELETE FROM employees where employee_id in (7);
