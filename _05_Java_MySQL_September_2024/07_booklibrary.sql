...
select reverse(replace(title,'The','***')) as Title, char_length(title) as 'Title length' 
from books 
where substring(title,1,3) = 'The';

select concat(first_name,' ', last_name) as 'Full name', timestampdiff(day,born, if(died is null,now(),died)) as 'Days Lived' from authors;

select date_format(now(),'%D-%b-%Y') as Current;
