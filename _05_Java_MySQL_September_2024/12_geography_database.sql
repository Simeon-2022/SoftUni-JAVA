...
modify person_id int not null unique auto_increment;

-- alter table people drop primary key;
-- alter table people modify person_id int not null unique auto_increment;

alter table people
add constraint fk_people_passport foreign key(passport_id) references passports(passport_id);

-- alter table passports drop constraint fk_people_passport ; 
insert into passports(passport_id, passport_number) value
(101, 'N34FG21B'),
(102, 'K65LO4R7'),
(103, 'ZE657QP2');
...
create table manufacturers (
	manufacturer_id int primary key not null auto_increment unique,
...
create table models (
	...
	constraint fk_manuf_model foreign key(manufacturer_id) references manufacturers(manufacturer_id) 
);
...
create table exams(
	exam_id int primary key not null auto_increment unique,
    name varchar(20)
);
...
create table cities(
	city_id int not null primary key auto_increment unique,
    name varchar(50)
);
...
create table item_types(
	item_type_id int not null primary key auto_increment unique,
    name varchar(50) 
);
...
create table items(
	...
	constraint fk_items foreign key(item_type_id) references item_types(item_type_id)
);
...
create table order_items(
    ...
    constraint fk_items1 foreign key(order_id) references orders(order_id),
    constraint fk_items2 foreign key(item_id) references items(item_id)
);

-- 06. University Database
create table subjects(
	subject_id int not null primary key auto_increment unique,
    subject_name varchar(50)
);

create table majors(
	major_id int not null primary key auto_increment unique,
    name varchar(50)
);

create table students(
	student_id int not null primary key auto_increment unique,
...
create table payments(
	payment_id int not null primary key auto_increment unique,
    payment_date date,
...
join mountains m 
on p.mountain_id = m.id where m.id = 17 -- mountain_range = 'Rila'
