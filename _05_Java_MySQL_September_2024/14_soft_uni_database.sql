...
-- 01. Employee Address
select employee_id, job_title, s.address_id, address_text
from employees e
inner join addresses s
on e.address_id = s.address_id
order by address_id
limit 5;
...
from employees e
inner join addresses a on a.address_id = e.address_id 
...
inner join departments d
on e.department_id = d.department_id
where d.name in ("Sales" ,"Finance") and hire_date > '1999-01-01' 
order by hire_date;
...
-- 09. Employee Manager
select e.employee_id, e.first_name, e.manager_id, (select first_name from employees where e.manager_id = employee_id) as manager_name 
from employees e
...
select  employee_id, 
		...
		(select concat(first_name, ' ', last_name) from employees where e.manager_id = employee_id) as manager_name, 
...
select avg(salary) as min_average_salary from employees group by department_id order by avg(salary) limit 1; 
