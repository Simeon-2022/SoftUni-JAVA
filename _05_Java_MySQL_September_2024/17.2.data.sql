...
-- alter table preserves modify column id int AUTO_INCREMENT,
-- add key(id);
...
select w.id as worker_id, first_name, last_name , p.name as preserve_name, c.country_code as country_code 
from workers w
...
join countries c on c.id = cp.country_id
where date_format(established_on, '%M') = 'May'
...
deterministic
begin
	declare avg_salary decimal(12,2);
...
select w.first_name, w.last_name, round(w.salary * 1.05,2) as 'salary after'
from workers w
join preserves p on p.id = w.preserve_id
join countries_preserves cp on w.preserve_id = cp.preserve_id
join countries c on c.id = cp.country_id
where c.name = 'Germany' -- ;country_name;