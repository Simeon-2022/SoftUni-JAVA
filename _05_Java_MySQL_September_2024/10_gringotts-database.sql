...
select deposit_group, sum(deposit_amount) as total_sum from wizzard_deposits where magic_wand_creator = 'Ollivander family' group by deposit_group order by deposit_group;

-- 07. Deposits Filter
select deposit_group, sum(deposit_amount) as total_sum from wizzard_deposits where magic_wand_creator = 'Ollivander family' group by deposit_group having total_sum < 150000 order by total_sum desc;

-- 08. Deposit Charge
select deposit_group, magic_wand_creator, min(deposit_charge) as min_deposit_charge from wizzard_deposits group by deposit_group, magic_wand_creator order by magic_wand_creator, deposit_group; 

-- 09. Age Groups
select min(age), max(age) from wizzard_deposits;
...
select (
	case
        ...
        when age > 60 then '[61+]'
	end) as age_group, count(id) as wizard_count from wizzard_deposits group by age_group order by age_group;
    
-- 10. First Letter
select left(first_name,1) as first_letter from wizzard_deposits where deposit_group = 'Troll Chest' group by first_letter order by first_letter;
