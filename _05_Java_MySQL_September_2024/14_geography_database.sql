...
inner join mountains_countries mc on mc.mountain_id = m.id
...
inner join countries c on c.country_code = mc.country_code
...
inner join peaks p on p.mountain_id = m.id
left join countries_rivers cr on c.country_code = cr.country_code
inner join rivers r on cr.river_id = r.id
