...
begin 
    ...
    from employees 
    where salary > 35000 
    order by first_name, last_name, salary, employee_id;
end $$
...
delimiter $$
create procedure usp_get_towns_starting_with (v_town_name varchar(50))
begin
	select name as town_name 
    from towns 
    where name like concat(upper(v_town_name), '%') 
    order by town_name;
end $$
delimiter ;
...
delimiter $$
create procedure usp_get_employees_from_town(v_town_name varchar(50))
begin
	select first_name, last_name 
    from employees e
    join addresses a on e.address_id = a.address_id 
    join towns t on a.town_id = t.town_id 
    where t.name = v_town_name 
...
delimiter $$
-- select locate(SUBSTRING('EWEER', 1, 1), 'set_of_letters');
...
begin
	...
	declare start_count int default 1;
	set end_count = length(word);
					label1: while start_count <= end_count do 
						set result = locate(SUBSTRING(word, start_count, 1),set_of_letters);
						if result > 0 then 
							set result = 1;
						else
                            leave label1;
